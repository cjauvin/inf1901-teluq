...
Deux plans, deux histoires — c'est le point de conception central :
  ...
  * la catégorie « vendue en moins de 30 jours » n'y est pas lisible. Elle se
    lit dans le plan **distance × année**, où elle forme deux amas franchement
    détachés : proche et récente d'un côté, loin et ancienne de l'autre.

Il faut bien deux renseignements discriminants pour obtenir des amas compacts :
avec un seul, chaque groupe s'étale sur toute la largeur de l'autre axe et l'on
n'obtient que deux bandes accolées. C'est pourquoi le registre porte la distance
au centre en plus de l'année — et pourquoi ni l'une ni l'autre n'intervient dans
le prix, qui ne suit que la superficie.
...
# ── Repères ───────────────────────────────────────────────────────────────────
# Plan « prix »     : superficie 100 m² → x=80, puis 2,894 px/m² ;
#                     prix 300 k$ → y=351,7 ; 500 k$ → 238,3 ; 700 k$ → 125.
# Plan « catégorie » : distance 0 km → x=80, puis 22 px/km ;
#                     année 1970 → y=332,7 ; 1990 → 223,6 ; 2010 → 114,5.
def px(m2):
...
def pa(annee):
    return 360 - (annee - 1965) * 5.4545


def cadre(hauteur, plan="prix"):
    """Le fond, la grille, les axes et leurs étiquettes."""
    if plan == "prix":
        crans = [("300 k$", 351.7), ("500 k$", 238.3), ("700 k$", 125)]
        titre_y = "prix de vente"
        crans_x = [("100", 80), ("150", 224.7), ("200", 369.5), ("250", 514.2)]
        titre_x = "superficie (m²)"
    else:
        crans = [("1970", 332.7), ("1990", 223.6), ("2010", 114.5)]
        titre_y = "année de construction"
        crans_x = [(str(k), pd(k)) for k in (0, 5, 10, 15, 20, 25)]
        titre_x = "distance du centre (km)"

    grille = "\n".join(f'<line x1="80" y1="{y}" x2="630" y2="{y}"/>' for _, y in crans)
    crans_x_lignes = "\n".join(
        f'<line x1="{x}" y1="380" x2="{x}" y2="386"/>' for _, x in crans_x)
    etiq_x = "\n".join(f'<text x="{x}" y="402">{lab}</text>' for lab, x in crans_x)
...
{etiq_x}
<text x="355" y="430" font-size="15" fill="{ENCRE}">{titre_x}</text>
</g>
...
def entete(hauteur, titre, desc, plan="prix", defs=""):
    return (
...
def coord(m, plan):
    return (px(m[0]), py(m[3])) if plan == "prix" else (pd(m[2]), pa(m[1]))


def points(couleur=TEAL, rayon=6.5, plan="prix", seulement=None):
    corps = "\n".join(
        "<circle cx=\"{:.1f}\" cy=\"{:.1f}\" r=\"{}\"/>".format(*coord(m, plan), rayon)
        for m in MAISONS
        if seulement is None or m[4] is seulement
    )
...
n = len(MAISONS)
MOYENNE = sum(m[3] for m in MAISONS) / n
m_moy = sum(m[0] for m in MAISONS) / n
pente = sum((m[0] - m_moy) * (m[3] - MOYENNE) for m in MAISONS) / sum(
...
def segments(vers_droite):
    ...
    corps = "\n".join(
        f'<line x1="{px(m[0]):.1f}" y1="{py(m[3]):.1f}" x2="{px(m[0]):.1f}" y2="{cible(m):.1f}"/>'
        for m in MAISONS
    )
...
def croix(demi=10, epaisseur=1.8):
    ...
    def branche(m):
        x, y = coord(m, "categorie")
        return (f'<path d="M{x - demi:.1f} {y - demi:.1f} L{x + demi:.1f} {y + demi:.1f} '
                f'M{x - demi:.1f} {y + demi:.1f} L{x + demi:.1f} {y - demi:.1f}"/>')

    corps = "\n".join(branche(m) for m in MAISONS if not m[4])
...
FIGURES["maisons-vendues"] = entete(
    ...
    "Les mêmes maisons, dans un tout autre plan : la distance au centre-ville en abscisse, l'année "
    "de construction en ordonnée. Chaque point est colorié selon la réponse à la "
...
FIGURES["maisons-erreurs-oui-non"] = entete(
    ...
    "Exactement le nuage de la page précédente — les mêmes maisons, aux mêmes places, la distance "
    "au centre-ville en abscisse et l'année de construction en ordonnée. Mais toutes sont maintenant "
...
if __name__ == "__main__":
    ...
    n_vite = sum(1 for m in MAISONS if m[4])
    ecart_plat = max(abs(m[3] - MOYENNE) for m in MAISONS)
    ecart_droite = max(abs(m[3] - ajuste(m[0])) for m in MAISONS)
