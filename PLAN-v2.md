...
**État au 2026-09-13** : arbre de travail **propre**, tout commité et poussé sur `v2`. Christian a relu jusqu’à *Prédire par ressemblance* inclus et attaque *Un modèle qui s’entraîne* ; restent ensuite *Classer*, *Généraliser*, *Trois façons d’apprendre*. La **régularisation** est écrite (section « Garder un modèle riche, mais le tenir en laisse », 3ᵉ volet du triptyque, figure `regularisation.svg` par `scripts/gen_regularisation.py` ; l’encart « énigme » la clôt). **Prochaine action de fond** : la page « Bien évaluer un modèle » (fuite de données, validation croisée, métriques), puis placer normalisation / arbres de décision / *hyperparamètre*, puis refaire les Objectifs du M2.
