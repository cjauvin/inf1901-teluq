...
Sheets](https://workspace.google.com/intl/en_ca/products/sheets/) au lieu du
choix plus courant Excel, car Google Sheets est plus accessible, et le langage
...
pour coller (plutôt que `Control + v` ou `Command + v`),
car ceci utilise la fonction "collage sans formatage", qui fait en sorte de ne
copier que l'information textuelle (en ignorant le formatage de la donnée
source). Si vous ne désirez pas utiliser les raccourcis clavier et que vous
utilisez un navigateur basé sur Chrome, veuillez noter qu'il vous sera
nécessaire d'installer l'extension [Google Docs hors
...
Il est possible de partager de manière anonyme un fichier Google Sheets (qui ne
